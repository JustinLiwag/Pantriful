...
class StepSix extends Component {
  ...
  continue = e => {
    e.preventDefault();
    if(this.props.values.shoppingListTwo.length === 0 ) {
  ...
  createCategoryItems = category => {
    ...
    const result = [];
    const shoppingListTwoState = this.props.values.shoppingListTwo
  ...
  renderModal = () => {
    return (
      <div className={"z-10 bg-white h-full w-full absolute top-0 overflow-auto " + (this.state.modalOpen ? 'block' : 'hidden')}>
        ...
        <p className="my-4 text-center text-lg font-bold">Second Shopping List</p>
        <div>
          ...
          <div className="flex justify-between sm:max-w-lg px-8 sm:px-4 mt-6 mx-auto">
            ...
            <p className="text-sm sm:text-lg text-gray-600 font-bold mb-48">Est. Total: <span className="text-green-button">{this.props.getTotal("shoppingListTwo")}</span></p>
          </div>
        </div>
      </div>
    )
  ...
  render() {
    return (
      <div>
        <Navbar/>
        <h3 className="mx-12 mt-8 text-lg font-bold text-gray-700 text-center md:text-2xl">Create your <span className="text-orange-base">second</span> example shopping list.</h3>
          ...
          <div className="hidden md:block w-1/2">
            <div>
              ...
              <div className="flex justify-between items-center px-8 mt-6">
                ...
                <p className="border-t-2 border-gray-200 md:border-none text-gray-500 font-bold">Est. Total: <span className="text-green-button">{this.props.getTotal("shoppingListTwo")}</span></p>
              </div>
          </div>
        </div>
        <Footer continue={this.continue} back={this.back} modal={true} open={this.toggleModal} length={this.props.values.shoppingListTwo.length}/>
      </div>
    );
  }
}
