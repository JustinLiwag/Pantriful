...
class Chicken extends Component {
    ...
    createCheckboxesFromApi = (items, category) => {
        ...
        filteredList.map(
            item=> (
                results.push(
                <li key={item.name} className="mt-2 mx-1 w-2/3 md:w-1/4 flex-grow">
                    <input 
                        className="hidden"
                        id={item.name}
                        type="checkbox"
                    ...
                    />
                    <label className="block text-gray-500 py-2 md:py-4 border-2 rounded border-gray-500 md:hover:bg-orange-200 md:hover:border-orange-base md:hover:text-orange-600 font-bold capitalize text-sm" htmlFor={item.name}>{item.name}</label>
                </li>
            )
        )
        )
    ...
    return (
        ...
        <div className="container">
            <h3 className="text-2xl md:text-3xl mt-4 md:mt-0 font-bold text-gray-600">Pick out your favorite cuts of <span className="text-orange-base">Poultry</span>.</h3>
